...
namespace eshopAPI.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            {
                var userRoles = UserManager.GetRoles(int.Parse(HttpContext.Current.User.Identity.GetUserId().ToString()));
                //if (userRoles[0] != "Admin")
                    //model.Firma_ID = int.Parse(base.GetClaimsValue("FirmaID"));
                
                var userRole = roleManager.FindByName(userRoles[0]);
                var newUserRole = roleManager.FindByName(model.UserRole.Name);
                if (userRoles[0] != "Admin" && userRole.Id > newUserRole.Id)
                    return BadRequest();
            }
            if (!ModelState.IsValid)
            {
            ...
            var user = new ApplicationUser() { UserName = model.Email, Email = model.Email, FirstName = model.Firstname, LastName = model.Lastname, LanguageID = model.Language_ID};
            ...
            var role = (HttpContext.Current.User.Identity.GetUserId() != null) ? roleManager.FindByName(model.UserRole.Name) : roleManager.FindByName("Admin");
            if (role != null)
